...
int main(int argc, char **argv)
{
    ...
    //Mat img1 = imread("/home/shuchun/SlamDemo/orbextrator/1341846313.553992yuantu.png",1);
    //Mat mask1 = imread("/home/shuchun/SlamDemo/orbextrator/img1_mask.png",0);
    ...
    for (size_t i = 0; i < nImages; i++)
    {
        ...
        mask1 = cv::imread(string(argv[5])+"/"+vstrImageFilenamesRGB[i],0);
        ...
        //imRGB = cv::imread("/home/shuchun/SlamDemo/orbextrator/1341846313.592026yuantu.png");
        //mask1 = cv::imread("/home/shuchun/SlamDemo/orbextrator/img1_mask.png",0);
        cv::imshow("imRGB",imRGB);
        cv::waitKey(0);
        cv::Mat img1 = imRGB.clone();
        Mat orig = img1.clone();
        ...
        cv::Mat mDescriptors;
        cv::Mat outimg;
        (*mpORBextractorLeft)(img1,cv::Mat(),keypoints1,mDescriptors);  //提取mask之后的图像特征点，此时边缘还没有处理

    //为了对比没有处理动态物体，这一块用原图测试一下
    //to::start   
    cv::Mat mDescriptors_ori;
    cv::Mat outimg_ori;
    std::vector<KeyPoint> keypoints_ori;

    (*mpORBextractorLeft)(imGray,cv::Mat(),keypoints_ori,mDescriptors_ori);
    cv::Mat ori = imRGB.clone(); 
    const float r = 5;
    const float n = keypoints_ori.size();  
    for (int i = 0; i < n; i++)
    {
      cv::Point2f pt1,pt2;
      pt1.x = keypoints_ori[i].pt.x-r;
      pt1.y = keypoints_ori[i].pt.y-r;
      pt2.x = keypoints_ori[i].pt.x+r;
      pt2.y = keypoints_ori[i].pt.y+r;
    
      cv::rectangle(imRGB,pt1,pt2,cv::Scalar(0,255,0));
      cv::circle(imRGB,keypoints_ori[i].pt,2,cv::Scalar(0,255,0),-1);
    }
    //drawKeypoints(ori,keypoints_ori,outimg_ori,Scalar::all(-1));
    imshow("ORB_SLAM2_ori", imRGB);
    waitKey(0);
    ...
    //erode
        ...
        //drawKeypoints(orig,keypoints1,outimg,Scalar::all(-1));
        //drawKeypoints(img1,keypoints1,outimg,Scalar::all(-1));//展示在mask上
    const float r1 = 5;
    const float n1 = keypoints1.size();  
    for (int i = 0; i < n1; i++)
    {
      cv::Point2f pt1,pt2;
      ...
      pt2.y = keypoints1[i].pt.y+r;
        ...
        cv::rectangle(img1,pt1,pt2,cv::Scalar(0,255,0));
        cv::circle(img1,keypoints1[i].pt,2,cv::Scalar(0,255,0),-1); 
        
    }

        cout<<"ORBSLAM2: "<<keypoints1.size()<<endl;
        imshow("ORB_SLAM2", img1);
        ...
        //waitKey(0);
        std::cout<< "number of frame = "  << i << std::endl;
    }
